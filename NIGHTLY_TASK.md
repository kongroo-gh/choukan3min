...
render.py は PNG と一緒に `instagram.txt` / `threads.txt` / `bluesky.txt` も
`content.json` の captions から書き出します。ここは自動なので何もしなくて構いません。

**`bluesky.alt.txt` だけは手で書いてください。**画像の代替テキストで、1行1枚・全4行
（Bluesky は4枚版）。原稿から機械的に作れないので自動化していません。順番は
1行目=表紙、2行目=何が起きたか、3行目=数字、4行目=影響です（3枚目の「なぜ」は
4枚版では落ちます）。**原稿を直したら代替テキストも直すこと。**行数が合っていても
中身の古さは検知できません（2026-08-29 で、見出しを修正したのに代替テキストの
1行目が修正前のまま残りました）。
