...
毎晩の手順は **`MORNING.md`**。別の Claude アカウントや別の機械へ移すときは
**`MIGRATE.md`**。定期タスクの指示文は **`NIGHTLY_TASK.md`** にある。
（`HANDOFF.md` と `SETUP.md` は初期構築時のもので、一部古い。）
...
6. **見出しで因果を断定しない。煽らない。**（2026-08-29 の回で踏んだ）
   本文が `research.md` の留保（「とみられる」「推計の域を出ない」）を保っているのに、
   見出しだけが言い切る、という形で起きやすい。見出しも本文と同じ基準で扱うこと。
   原則は**起きた順番や事実の並びを述べ、原因の断定を避ける**。問いかけの形も使える。
   悪い例: 「タダにしたら、入れなくなった」「無償化が需要を呼んだ」
   良い例: 「無償化のあとで、待機児童が増えた」「なぜ東京だけ増えたか」
   `validate.py` の禁止語リストは最低限の歯止めであって、そこに無ければ何を書いても
   よいという意味ではない。読者の不安を煽って注意を引く書き方はしない。
7. **免責文を消さない。** 各キャプション末尾の「※投資判断はご自身の責任で〜」は必須。
8. **5枚目の反対意見を消さない。** 断定だけの投稿にしない方針。
