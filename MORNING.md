...
PNG と一緒に `instagram.txt` / `threads.txt` / `bluesky.txt` を `content.json` の
`captions` から書き出します。**`bluesky.txt` は `post_bluesky.py` が読むので、無いと
翌朝の投稿だけが落ちます**（関門1も関門2も .txt の有無を見ないため、push は通ります）。
原稿を直したら render.py をかけ直せば .txt も揃います。

`bluesky.alt.txt`（代替テキスト・1行1枚）だけは原稿から機械的に作れないので**手で書きます。**
既にあるものは上書きしません。行数が画像の枚数と違うときだけ注意が出ます。
見出しを直したのに代替テキストが古いまま、というのは実際に起きています
（2026-08-29 の1行目が修正前の見出しのまま残りました）。行数が合っていても中身の
古さは検知できないので、原稿を直したら代替テキストも読み直してください。
